...
<jsp:useBean id="person" class="d28.Person"/>
<!DOCTYPE HTML>
<html>
<head>
...
<form action="/JSTL_if_tag.jsp" method="post">
    <input type="text" name="uname" value="${person.name}">
    <input type="submit" value="登录">
</form>
<c:set target="${person}" property="name">admin</c:set>
<%--使用if标签进行判断并把检验后的结果赋给adminchock，存储在默认的page范围中。 --%>
<c:if test="${person.name=='admin'}" var="adminchock">
